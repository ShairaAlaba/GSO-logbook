      ...
      <div v-for="(rec, ri) in filteredRecords" :key="rec.id" class="record-card">
        ...
        <div class="record-card-body" :class="{ open: openRecords[rec.id] }">
          ...
          <div v-if="editingId[rec.id]" class="edit-notice no-print">
            Editing mode - make your changes then click Save.
          </div>
        </div>
        <div class="signature-block">
          <div class="sig-item">
            <div class="sig-line"></div>
            <div class="sig-name">ACE-ME CASINO</div>
...
const filteredRecords = computed(() => {
  return store.powInspectionRecords.filter(rec => {
    ...
    return matchSearch && matchYear && matchType && rec.rows.length > 0
  })
})
...
.btn-warning { background:#f59e0b; color:#000; border:1.5px solid #d97706; }
