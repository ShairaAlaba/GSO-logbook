<template>
  <div>
    <div class="page-header no-print">
      <div>
        <h1>Create New POW Inspection Record</h1>
        <div class="breadcrumb">POW Monitoring → Pre / Post Inspection → Create New Record</div>
      </div>
    </div>
    ...
    <div class="page-content">
        ...
        <div class="card-body">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            <div class="form-group">
              ...
              <input class="form-control" v-model="form.title" placeholder="e.g. February 2026 POW Inspection Log" />
            </div>
            <div class="form-group">
      ...
      <div class="no-print" style="display:flex;gap:12px;margin-top:16px;">
        ...
        <button class="btn btn-secondary btn-lg" @click="$router.push('/pow-inspection/all')">Cancel</button>
      </div>

      <div v-if="saved" style="margin-top:16px;padding:12px 16px;background:#d1fae5;border-radius:6px;color:#065f46;font-weight:500;">
        Record saved! <router-link to="/pow-inspection/all" style="color:#047857;">View All Records →</router-link>
      </div>
    </div>
  </div>
</template>
...
function saveRecord() {
  ...
  store.addPowInspectionRecord({
    id: `POW-INSP-${form.type.toUpperCase()}-${Date.now()}`,
    title: form.title,
    type: form.type,
    year: form.year,
    month: form.month,
