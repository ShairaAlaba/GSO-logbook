  ...
  <div>
      ...
      <div v-for="rec in filteredRecords" :key="rec.id" class="record-card">
        <div class="record-card-header" @click="toggleRecord(rec.id)">
          ...
          <div class="record-card-actions">
            ...
            <button class="btn btn-excel btn-sm" @click.stop="exportToExcel(rec)">⬇ Excel</button>
    ...
    <div v-for="rec in filteredRecords" :key="'p-' + rec.id" class="pow-print-doc" :data-print-id="rec.id">
      ...
      <div class="sum-stat-bar">
        ...
        <div class="sum-stat">
          <div class="sum-stat-num">{{ countStatus(rec, 'Confirm') }}</div>
          <div class="sum-stat-lbl">Confirmed</div>
        </div>
        <div class="sum-stat">
        ...
        <div class="sum-stat">
          <div class="sum-stat-num">{{ countStatus(rec, 'Disapprove') }}</div>
          <div class="sum-stat-lbl">Disapproved</div>
        </div>
        <div class="sum-stat">
          ...
          <table class="sum-breakdown-table">
            <thead>
              <tr>
                <th>Office</th>
                <th>Total</th>
                <th style="color:#16a34a;">Confirmed</th>
                <th style="color:#dc2626;">Cancelled</th>
                <th style="color:#b45309;">Disapproved</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="o in officeBreakdown(rec)" :key="o.name">
                ...
                <td style="text-align:center">{{ o.confirmed || 0 }}</td>
                <td style="text-align:center">{{ o.cancelled || 0 }}</td>
                <td style="text-align:center">{{ o.disapproved || 0 }}</td>
              </tr>
              <!-- Totals row -->
              <tr style="font-weight:700; background:#e8f5e9;">
                <td>TOTAL</td>
                <td style="text-align:center">{{ officeBreakdown(rec).reduce((s,o)=>s+o.total,0) }}</td>
                <td style="text-align:center">{{ officeBreakdown(rec).reduce((s,o)=>s+(o.confirmed||0),0) }}</td>
                <td style="text-align:center">{{ officeBreakdown(rec).reduce((s,o)=>s+(o.cancelled||0),0) }}</td>
                <td style="text-align:center">{{ officeBreakdown(rec).reduce((s,o)=>s+(o.disapproved||0),0) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
function officeBreakdown(rec) {
  ...
  nonEmptyRows(rec).forEach(row => {
    const o = row[7] || ''
    if (!map[o]) map[o] = { name: o, total: 0, confirmed: 0, cancelled: 0, disapproved: 0 }
    map[o].total++
    const status = String(row[col] || '').toLowerCase()
    if (status.includes('confirm')) map[o].confirmed++
    else if (status.includes('cancel')) map[o].cancelled++
    else if (status.includes('disapprove')) map[o].disapproved++
  })
...
async function exportToExcel(rec) {
  // Dynamically load xlsx-js-style (supports cell-level styling)
  if (!window.XLSXStyle) {
  ...
  // ── Theme colors ──
  const GREEN_DARK  = '2d6127'
  const GREEN_MID   = '3b8132'
  const GREEN_LIGHT = 'e8f5e9'
  const WHITE       = 'FFFFFF'

  const hdrFont  = { bold: true, color: { rgb: WHITE }, name: 'Arial', sz: 9 }
  const subFont  = { bold: true, color: { rgb: WHITE }, name: 'Arial', sz: 8 }
  const thinBorder = {
  ...
  // ── ROW 0: Title banner ──
  sc(R, 0, 'VEHICLE RESERVATION LOGBOOK', {
    font: { bold: true, color: { rgb: WHITE }, name: 'Arial', sz: 13 },
    fill: { fgColor: { rgb: GREEN_DARK }, patternType: 'solid' },
    alignment: center, border: thinBorder,
  })
  merges.push({ s: { r: R, c: 0 }, e: { r: R, c: NUM_COLS - 1 } })
  R++

  // ── ROW 1-3: Info block ──
  const lblStyle = { font: { bold: true, name: 'Arial', sz: 8 }, fill: { fgColor: { rgb: GREEN_LIGHT }, patternType: 'solid' }, border: thinBorder, alignment: left }
  const valStyle = { font: { name: 'Arial', sz: 8 }, border: thinBorder, alignment: left }

  const infoRows = [
    ['Logbook Title', rec.title, '', ''],
    ['Vehicle', rec.vehicle, 'Year', rec.year],
    ['Recorded By', rec.recordedBy, 'Date Exported', today],
  ]
  for (const [l1, v1, l2, v2] of infoRows) {
    sc(R, 0, l1, lblStyle); sc(R, 1, v1, valStyle)
    merges.push({ s: { r: R, c: 1 }, e: { r: R, c: 6 } })
    sc(R, 7, l2, lblStyle); sc(R, 8, v2, valStyle)
    merges.push({ s: { r: R, c: 8 }, e: { r: R, c: NUM_COLS - 1 } })
    R++
  }
  R++ // spacer

  // ── SUMMARY STATS ──
  sc(R, 0, 'RECORD SUMMARY', {
    font: { bold: true, color: { rgb: WHITE }, name: 'Arial', sz: 9 },
    fill: { fgColor: { rgb: GREEN_MID }, patternType: 'solid' },
    alignment: center, border: thinBorder,
  })
  merges.push({ s: { r: R, c: 0 }, e: { r: R, c: NUM_COLS - 1 } })
  R++
  ...
  const stats = [
    ['Total Trips', nonEmptyRows(rec).length],
    ['Completed',   countStatus(rec, 'Completed')],
    ['Confirmed',   countStatus(rec, 'Confirm')],
    ['Pending',     countStatus(rec, 'Pending')],
    ['Cancelled',   countStatus(rec, 'Cancel')],
    ['Disapproved', countStatus(rec, 'Disapprove')],
    ['Drivers',     uniqueDrivers(rec).length],
    ['Offices',     uniqueOffices(rec).length],
  ]
  const statWidth = Math.floor(NUM_COLS / stats.length)
  stats.forEach(([lbl, val], i) => {
    const col = i * statWidth
    sc(R,   col, lbl, { font: { bold: true, color: { rgb: '1a6e1a' }, name: 'Arial', sz: 7 }, fill: { fgColor: { rgb: GREEN_LIGHT }, patternType: 'solid' }, border: thinBorder, alignment: center })
    sc(R+1, col, val, { font: { bold: true, color: { rgb: '1a6e1a' }, name: 'Arial', sz: 13 }, fill: { fgColor: { rgb: GREEN_LIGHT }, patternType: 'solid' }, border: thinBorder, alignment: center })
  })
  R += 2
  R++ // spacer

  // ── COMPLETE TRIP RECORDS heading ──
  sc(R, 0, 'COMPLETE TRIP RECORDS', {
    font: { bold: true, color: { rgb: WHITE }, name: 'Arial', sz: 9 },
    fill: { fgColor: { rgb: GREEN_MID }, patternType: 'solid' },
    alignment: center, border: thinBorder,
  })
  ...
  const groups = rec.groups || defaultGroups
  let col = 0
  for (const g of groups) {
    const span = g.children ? g.children.length : 1
    sc(R, col, g.label, {
      font: hdrFont,
      fill: { fgColor: { rgb: GREEN_DARK }, patternType: 'solid' },
      border: thinBorder, alignment: center,
    })
    if (span > 1) merges.push({ s: { r: R, c: col }, e: { r: R, c: col + span - 1 } })
    col += span
  }
  R++

  // ── Column group sub-headers (row 2) ──
  col = 0
  ...
  for (const g of groups) {
    if (g.children) {
      ...
      for (const child of g.children) {
        sc(R, col, child, {
          font: subFont,
          fill: { fgColor: { rgb: '4a9e40' }, patternType: 'solid' },
          border: thinBorder, alignment: center,
        })
        col++
      }
    } else {
      merges.push({ s: { r: R - 1, c: col }, e: { r: R, c: col } })
      col++
    }
  }
  if (hasChildren) R++
  ...
  // ── Data rows ──
  const altFill = { fgColor: { rgb: 'f0f7ee' }, patternType: 'solid' }
  const whtFill = { fgColor: { rgb: WHITE }, patternType: 'solid' }
  nonEmptyRows(rec).forEach((row, ri) => {
    const fill = ri % 2 === 1 ? altFill : whtFill
    row.forEach((cell, ci) => {
      sc(R, ci, cell ?? '', { font: { name: 'Arial', sz: 7 }, fill, border: thinBorder, alignment: left })
    })
    R++
  })
  ...
  // ── Signatures ──
  R += 2
  sc(R, 1, 'Prepared By:', { font: { bold: true, name: 'Arial', sz: 8 }, border: thinBorder, alignment: center })
  merges.push({ s: { r: R, c: 1 }, e: { r: R, c: 5 } })
  sc(R, 9, 'Approved By:', { font: { bold: true, name: 'Arial', sz: 8 }, border: thinBorder, alignment: center })
  merges.push({ s: { r: R, c: 9 }, e: { r: R, c: NUM_COLS - 1 } })
  R += 3
  sc(R, 1, 'ACE-ME CASIÑO', { font: { bold: true, name: 'Arial', sz: 8 }, border: thinBorder, alignment: center })
  merges.push({ s: { r: R, c: 1 }, e: { r: R, c: 5 } })
  sc(R, 9, 'ENGR. MARIA M. DELO', { font: { bold: true, name: 'Arial', sz: 8 }, border: thinBorder, alignment: center })
  merges.push({ s: { r: R, c: 9 }, e: { r: R, c: NUM_COLS - 1 } })

  // ── Sheet settings ──
  ws['!ref'] = XS.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: R, c: NUM_COLS - 1 } })
  ws['!merges'] = merges
  ws['!cols'] = [
    { wch: 10 }, // TRIP TICKET
    { wch: 18 }, // DRIVER
  ...
  const sheetName = (rec.vehicle || 'Record').substring(0, 31)
  XS.utils.book_append_sheet(wb, ws, sheetName)

  const filename = `${rec.title.replace(/[^a-zA-Z0-9 _\-]/g, '_')}.xlsx`
...
function printRecord(id) {
  ...
  style.id = '__print-target-style__'
  style.textContent = `.pow-print-doc:not([data-print-id="${id}"]) { display: none !important; }`
  document.head.appendChild(style)
  setTimeout(() => {
    window.print()
    setTimeout(() => { document.getElementById('__print-target-style__')?.remove() }, 500)
  }, 300)
}

function printSearchResults() {
  document.getElementById('__print-target-style__')?.remove()
...
.btn-excel   { background:#1d6f42; color:#fff; border:1.5px solid #145a34; }
...
@media print {
  ...
  .pow-print-doc { display: block !important; }
  .pow-print-doc:first-of-type { page-break-before: avoid; break-before: avoid; }

  /* Letterhead */
  .pow-letterhead { display: flex !important; align-items: center; justify-content: space-between; padding-bottom: 4pt; border-bottom: 1.5pt solid #2d6127; margin-bottom: 4pt; }
  .pow-logo { height: 55pt; width: auto; object-fit: contain; }
  .pow-lh-center { display: none; }
  ...
  /* Full data table */
  .sum-data-table { width: 100%; border-collapse: collapse; font-size: 5.5pt; margin-top: 2pt; table-layout: fixed; }
  /* Column widths for reservation print table */
  .sum-data-table th:nth-child(1)  { width: 5%; }   /* TRIP TICKET NO. */
  .sum-data-table th:nth-child(2)  { width: 8%; }   /* ASSIGNED DRIVER */
  .sum-data-table th:nth-child(3)  { width: 7%; }   /* MONTH & DATE/S */
  .sum-data-table th:nth-child(4)  { width: 4%; }   /* TIME */
  .sum-data-table th:nth-child(5)  { width: 9%; }   /* DESTINATION */
  .sum-data-table th:nth-child(6)  { width: 11%; }  /* PURPOSE */
  .sum-data-table th:nth-child(7)  { width: 6%; }   /* REQUESTER'S CONTACT NO. */
  .sum-data-table th:nth-child(8)  { width: 9%; }   /* REQUESTING OFFICE */
  .sum-data-table th:nth-child(9)  { width: 7%; }   /* REMARKS */
  .sum-data-table th:nth-child(10) { width: 7%; }   /* PER DIEM */
  .sum-data-table th:nth-child(11) { width: 7%; }   /* FUEL */
  .sum-data-table th:nth-child(12) { width: 5%; }   /* STATUS */
  .sum-data-table th:nth-child(13) { width: 7%; }   /* RECORDED BY */
  .sum-data-table th:nth-child(14) { width: 8%; }   /* UPDATED BY */

  .sum-data-table th {
    background: #3b8132 !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important;
    color: #fff !important; font-weight: 600; padding: 2pt 3pt;
    border: 0.4pt solid #2d6127; text-align: center; white-space: normal; word-break: break-word;
  }
